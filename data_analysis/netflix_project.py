...
# .replace(no.nan, 'b') : 결측치를 문자열 바꾸기 함수를 통해 처리
...
plt.pie(type_count, labels=type_counts.index, autopct='%0.f%%', startangle=100,
...
# [3단계] .value_count()를 붙여 장르의 등장 횟수 계산
# 예시) 인덱스 1의 listed_in 열 값 : International TV Shows, TV Dramas, TV Mysteries
genres = netflix['listed_in'].str.split(',', expand=True).stack().value_counts()
genres
...
netflix_age_country_unstack = netflix_age_country.unstack.loc[age_order, country_order]
...
wordcloud = WpordCloud(background_color='white', width=1400, height=1400,
                    max_words=170, mask=mask, colormap=cmap).generate(text)

plt.suptitle('Keywords in the description of Movies and TV ')